...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(rd) = SEXT(BITS(imm + src1, 31, 0), 32));
  INSTPAT("000000? ????? ????? 001 ????? 00110 11", slliw  , I, if (!(imm & 0b100000)) R(rd) = SEXT(BITS(src1 << imm, 31, 0), 32));
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, if (!(imm & 0b100000)) R(rd) = SEXT((((int32_t)BITS(src1, 31, 0))) >> BITS(imm, 5, 0), 32));
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw  , I, if (!(imm & 0b100000)) { R(rd) = SEXT((BITS(src1, 31, 0)) >> imm, 32); });

  INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld     , I, R(rd) = Mr(imm + src1, 8));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = SEXT((int32_t)src1 >> BITS(src2, 5, 0), 32));
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm );
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(rd) = SEXT(BITS(src1 << BITS(src2, 4, 0), 31, 0), 32) );
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", slrw   , R, R(rd) = SEXT(BITS(src1, 31, 0) >> BITS(src2, 4, 0), 32) );
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, if ((int32_t)BITS(src2, 31, 0) == 0) { R(rd) = -1; } else if (((int32_t)BITS(src1, 31, 0) == (1 << 31)) && ((int32_t)BITS(src2, 31, 0) == -1)) { R(rd) = SEXT(1 << 31, 32);} else R(rd) = ((int32_t)BITS(src1, 31, 0)) / ((int32_t)BITS(src2, 31, 0)));
