  ...
  while (e[position] != '\0') {
    ...
    for (i = 0; i < NR_REGEX; i++) {
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 &&
        ...
        // Log("%d: match rules[%d] = \"%s\" at position %d with len %d: %.*s,
        // token type: %d",
        //     nr_token, i, rules[i].regex, position, substr_len, substr_len,
        //     substr_start, rules[i].token_type);
        ...
        switch (rules[i].token_type) {
          ...
          case TK_HEX:
          case TK_DECIMAL:
            Assert(substr_len < 32, "token str too long!");
...
int find_position(int p, int q) {
  ...
  for (int i = q; i >= p; i--) {
    if ((tokens[i].type == TK_AND) && rp == 0 && index[0] == -1) {
      ...
      rp--;
    } else if ((tokens[i].type == '+' || tokens[i].type == '-') && rp == 0 &&
               index[2] == -1) {
      index[2] = i;
    } else if ((tokens[i].type == '*' || tokens[i].type == '/') && rp == 0 &&
               index[3] == -1) {
...
word_t eval(int p, int q) {
  if (p > q) {
    ...
    // Assert(tokens[p].type == TK_DECIMAL && tokens[q].type == TK_DECIMAL, "p
    // and q should all be decimal ints!"); 4 +3*(2- 1)
...
word_t expr(char *e, bool *success) {
  ...
  for (int i = 0; i < nr_token; i++) {
    if (tokens[i].type == '*' &&
        (i == 0 ||
         (tokens[i - 1].type != TK_REGS && tokens[i - 1].type != TK_HEX &&
          tokens[i - 1].type != TK_DECIMAL && tokens[i - 1].type != ')'))) {
      tokens[i].type = TK_DEREF;
    } else if (tokens[i].type == '-' &&
               (i == 0 || (tokens[i - 1].type != TK_REGS &&
